"""
FastAPI Application for Domain Analysis - UPDATED VERSION
Cloud Run deployment with BigQuery integration
FIXES: 1) Email-based duplicates, 2) CSV processing, 3) Queue processing
"""
...
# FIXED: Email-based processing instead of domain-based
async def process_email_analysis_fixed(
    email: str, 
    analyzer: DomainAnalyzer, 
    bq_client: BigQueryClient,
    force_refresh: bool = False
) -> AnalysisResult:
    """FIXED: Process single email with EMAIL-based duplicate detection"""
    
    # FIXED: Check if this EXACT EMAIL was processed recently (not domain)
    if not force_refresh:
        ...
        if existing_emails.get(email, False):
            ...
            # Query by exact email, not domain
            ...
            cached_df = query_job.result().to_dataframe()
...
# FIXED: Robust CSV processing with better error handling
async def process_csv_emails_background_fixed(
    session_id: str, 
    emails: List[str], 
    analyzer: DomainAnalyzer, 
    bq_client: BigQueryClient
):
    """FIXED: More robust background processing"""
    logger.info(f"FIXED Background processing started for session {session_id} with {len(emails)} emails")
    try:
        session = get_or_create_session(session_id)
        ...
        await send_chat_message(
            session_id, 
            f"🚀 Starting batch processing of {total_emails} emails. Progress updates every 10 emails."
        )
        
        for i, email in enumerate(emails):
            try:
                logger.info(f"Processing email {i+1}/{total_emails}: {email}")
                
                # Use FIXED email-based duplicate detection
                existing_emails = bq_client.check_multiple_emails_exist([email], max_age_hours=24)
                
                if existing_emails.get(email, False):
                    duplicates += 1
                    logger.info(f"Email {email} was already processed (cached)")
                    processed += 1
                    continue
                ...
                if result.scraping_status in ['success', 'success_text', 'success_fallback']:
                    successful += 1
                    logger.info(f"Email {email} processed successfully")
                else:
                    failed += 1
                    logger.info(f"Email {email} processing failed: {result.scraping_status}")
                
                processed += 1
                
                # Send progress update every 10 completions or at the end
                if processed % 10 == 0 or processed == total_emails:
                    ...
                    logger.info(f"Progress update sent: {progress_msg}")
...
All results have been saved to the database. You can now submit more emails or upload another CSV file!"""
        ...
        await send_chat_message(session_id, final_msg, {
            "batch_results": {
                ...
                "results": [r.dict() for r in all_results[-10:]] if all_results else []
            }
        })
        
        logger.info(f"CSV processing completed successfully for session {session_id}")
        
    except Exception as e:
        logger.error(f"CRITICAL ERROR in CSV background processing: {str(e)}")
        import traceback
        logger.error(f"Full traceback: {traceback.format_exc()}")
        
        error_msg = f"❌ Batch processing failed: {str(e)}. Please try uploading a smaller file or contact support."
...
async def root():
    ...
    return {
        "name": "Domain Analysis API - FIXED VERSION",
        "version": "1.1.0",
        "description": "Advanced RAG pipeline for email domain intelligence gathering",
        "fixes": [
            "Email-based duplicate detection (not domain-based)",
            "Fixed CSV processing background tasks",
            "Working BigQuery queue processing"
        ],
        "endpoints": {
            ...
            "chat": "WebSocket /ws/{session_id} - Real-time chat",
            "csv_upload": "POST /chat/upload-csv - CSV batch processing"
        }
    }
...
async def analyze_email(
    ...
    """
    Analyze a single email address - FIXED with email-based duplicates
    """
    try:
        # Use FIXED processing function
        result = await process_email_analysis_fixed(
            email=request.email,
            analyzer=analyzer,
            bq_client=bq_client,
...
async def batch_analyze_emails(
    ...
    """
    Analyze multiple email addresses in batch - FIXED
    """
    try:
        ...
        # Process each email with FIXED logic
...
async def retry_failed_batch_items(
    ...
    try:
        ...
        # Restart FIXED background processing
        asyncio.create_task(process_massive_batch_background_fixed(batch_id, bq_client))
...
# WEBSOCKET AND CHAT ENDPOINTS - FIXED
...
async def send_chat_message_endpoint(
    ...
    """Handle chat messages (single email processing) - FIXED"""
    try:
        ...
        if email_match:
            ...
            # FIXED: Check for EMAIL duplicates, not domain duplicates
            existing_emails = bq_client.check_multiple_emails_exist([email], max_age_hours=24)
            ...
            # Process email with FIXED logic
            await send_chat_message(request.session_id, f"Processing email: {email}...")
            
            # Use FIXED processing function
            result = await process_email_analysis_fixed(email, analyzer, bq_client, force_refresh=False)
...
# CSV PROCESSING ENDPOINTS - FIXED
...
async def preview_csv_file(
    ...
    """Preview CSV file with FIXED BigQuery duplicate checking"""
    ...
    try:
        ...
        # FIXED: Clean emails with EMAIL-based duplicate checking
        valid_emails, cleaning_stats = clean_email_dataframe(df, bq_client)
...
async def upload_csv_file(
    ...
    """Handle CSV file upload and process emails in batch - FIXED"""
...
🚀 Starting analysis for {len(valid_emails)} new emails..."""
        ...
        # Process emails in background with FIXED function
        logger.info(f"Starting FIXED background processing for {len(valid_emails)} emails")
        task = asyncio.create_task(process_csv_emails_background_fixed(
...
# DEPLOYMENT VERIFICATION ENDPOINTS (for testing fixes)

@app.get("/test/email-duplicate-check")
async def test_email_duplicate_check(
    email1: str = "test1@example.com",
    email2: str = "test2@example.com",
    bq_client: BigQueryClient = Depends(get_bigquery_client)
):
    """
    Test endpoint to verify email-based duplicate detection is working
    """
    try:
        # Test email-based checking
        test_results = bq_client.check_multiple_emails_exist([email1, email2], max_age_hours=24)
        
        return {
            "test_type": "email_duplicate_check",
            "status": "FIXED - Using email-based detection",
            "test_emails": [email1, email2],
            "duplicate_status": test_results,
            "message": "This should check exact EMAIL matches, not domain matches"
        }
...
@app.get("/test/queue-status/{batch_id}")
async def test_queue_status(
    batch_id: str,
    bq_client: BigQueryClient = Depends(get_bigquery_client)
):
    """
    Test endpoint to check if queue processing is actually working
    """
    try:
        # Get queue items for this batch
        queue_items = bq_client.get_pending_queue_items(limit=50, priority_order=True)
        batch_items = [item for item in queue_items if item['batch_id'] == batch_id]
        ...
        return {
            "test_type": "queue_processing_check",
            "status": "FIXED - Queue processor should be working",
            "batch_id": batch_id,
            "batch_status": batch_status,
            "pending_queue_items": len(batch_items),
            "queue_items_sample": batch_items[:5],  # First 5 items
            "message": "Check if items are moving from 'pending' to 'processing' to 'completed'"
        }
...
@app.post("/test/csv-processing")
async def test_csv_processing(
    session_id: str = "test-session-123"
):
    """
    Test endpoint to verify CSV processing won't hang
    """
    try:
        # Create test data
        ...
        # Start background task (should not hang)
        task = asyncio.create_task(
            process_csv_emails_background_fixed(session_id, test_emails, analyzer, bq_client)
        )
        
        return {
            "test_type": "csv_processing_check",
            "status": "FIXED - Background task started without hanging",
            "session_id": session_id,
            "test_emails": test_emails,
            "task_created": True,
            "message": "CSV processing should now work properly with progress updates via WebSocket"
        }
...
@app.get("/fixes-summary")
async def get_fixes_summary():
    """
    Summary of all fixes applied to resolve the three critical issues
    """
    return {
        "version": "1.1.0-FIXED",
        "fixes_applied": {
            "issue_1": {
                "problem": "Domain-based duplicate detection instead of email-based",
                "solution": "Changed all duplicate checking to use exact email matching",
                "affected_functions": [
                    "process_email_analysis_fixed",
                    "clean_email_dataframe", 
                    "send_chat_message_endpoint",
                    "process_csv_emails_background_fixed",
                    "process_massive_batch_background_fixed"
                ],
                "status": "FIXED"
            },
            "issue_2": {
                "problem": "CSV upload processing hangs indefinitely",
                "solution": "Enhanced error handling, proper asyncio task management, and progress reporting",
                "affected_functions": [
                    "process_csv_emails_background_fixed",
                    "upload_csv_file"
                ],
            ...
            "issue_3": {
                "problem": "BigQuery queue items created but never processed",
                "solution": "Fixed queue processing loop with proper item fetching and status updates",
                "affected_functions": [
                    "process_massive_batch_background_fixed",
                    "upload_massive_csv"
                ],
        ...
        "testing_endpoints": {
            "/test/email-duplicate-check": "Test email-based duplicate detection",
            "/test/queue-status/{batch_id}": "Check queue processing status", 
            "/test/csv-processing": "Test CSV background processing"
        },
        "deployment_notes": [
            "All functions now use email-based duplicate checking",
            "Background tasks have proper exception handling",
            "Queue processor runs in continuous loop",
            "Progress updates sent every 10 emails processed",
...
@app.get("/debug/session-status/{session_id}")
async def get_session_debug_info(session_id: str):
    """
    Debug endpoint to check session status and message history
    """
    try:
        if session_id in chat_sessions:
            ...
            return {
                ...
                "processing_status": session.processing_status.dict() if session.processing_status else None,
                "recent_messages": [
                    {
            ...
            return {
                ...
                "session_id": session_id,
                "message": "Session not found - will be created on first message"
            }
    except Exception as e:
        return {
            "error": str(e),
